...
u8 My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
	...
	if(RTC_ReadBackupRegister(RTC_BKP_DR0)!=0x5052)		//是否第一次配置?
	{
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(18,11,56,RTC_H12_AM);	//设置时间
		RTC_Set_Date(21,5,6,4);		//设置日期
	 
		RTC_WriteBackupRegister(RTC_BKP_DR0,0x5052);	//标记已经初始化过了
	} 
